    ...
    public class Matrix : IEnumerable, IFormattable, ICloneable
    {
        ...
        public Matrix(int Rows, int Columns, double[,] array)
        {
            ...
            if (Rows < 0 || Columns < 0)
            {
                ...
                throw new ArgumentOutOfRangeException($"Ошибка размерности: {Columns}", $"Недопустимое количество столбцов объекта: {Id}");
            }
        ...
        public Matrix(int Rows, int Columns, double[] array)
        {
            ...
            if (Rows <= 0 || Columns <= 0 || array.Length != Rows * Columns)
            {
                throw new ArgumentOutOfRangeException($"Ошибка размерности: {Rows}", $"Недопустимое количество строк объекта: {Id}");
                throw new ArgumentOutOfRangeException($"Ошибка размерности: {Columns}", $"Недопустимое количество столбцов объекта: {Id}");
                throw new ArgumentOutOfRangeException($"Ошибка длины массива {array.Length}", $"Недопустимое количество элементов матрицы объекта №{Id}: {Rows}x{Columns}");
            }
