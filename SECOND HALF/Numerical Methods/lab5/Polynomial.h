...
namespace Math
{
	...
	public:
		...
		{
			if (m_length < 0)
				throw  std::invalid_argument("Ñòåïåíü ïîëèíîìà îòðèöàòåëüíà.");
		...
		const Polynomial& operator/=(const Polynomial<T>& polynomial)
		{
			if (polynomial.m_coeff[polynomial.m_length - 1] == 0)
				throw std::logic_error("Äåëåíèå íà íîëü\n");
		...
		T& operator[](int index)
		{
			if (index >= static_cast<int>(m_length) || static_cast<int>(m_length) + index < 0)
				throw std::invalid_argument("Èíäåêñ ÷ëåíà ïîëèíîìà áîëüøå êîëè÷åñòâà ÷ëåíîâ");
			else if (index < 0)
				index += m_length;
